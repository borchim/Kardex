...
import { turkishRealEstateSections } from './templates/turkish-realestate-sections';
...
export default function LandingPageSettings() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<'default' | 'turkish-realestate'>('default');
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('Load Template')}</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">{t('Replace all sections with a pre-built market template. This will overwrite your current content.')}</p>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
                    <div className="flex-1 space-y-2">
                      <Label className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('Select Template')}</Label>
                      <select
                        value={selectedTemplate}
                        onChange={(e) => setSelectedTemplate(e.target.value as 'default' | 'turkish-realestate')}
                        className="w-full h-10 px-3 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm"
                      >
                        <option value="default">{t('Default — Generic SaaS')}</option>
                        <option value="turkish-realestate">🇹🇷 Türkiye Emlak — Turkish Real Estate</option>
                      </select>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      className="border-purple-300 text-purple-700 hover:bg-purple-50 dark:border-purple-600 dark:text-purple-400 shrink-0"
                      onClick={() => {
                        const tpl = selectedTemplate === 'turkish-realestate'
                          ? turkishRealEstateSections
                        ...
                        setData('config_sections', {
                          ...
                          section_visibility: tpl.section_visibility,
                          custom_pages: data.config_sections?.custom_pages || []
                        });
                      }}
                    >
